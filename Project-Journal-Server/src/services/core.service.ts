...
export async function SendForgotPasswordEmail(email: string) {
    const setTokenResult = await pool.query(
        "UPDATE jn.users SET verify_token = gen_random_uuid(), verify_token_expire = NOW() + INTERVAL '1 hour' WHERE email = $1 RETURNING verify_token",
        [email]
    );
    ...
    const verification_link = `${ENV.CLIENT_URL}/reset-password/${setTokenResult.rows[0].verify_token}`;
    const replacements: MailTemplateReplacements = {
        header: `<h1 class="logo">SafeTrade</h1>
                <p style="margin: 10px 0 0; opacity: 0.8; font-weight: 300;">Safe & Secure Computer Marketplace</p>`,
...
export async function ChangePassword(token: string, newPassword: string) {
    ...
    await pool.query("UPDATE jn.users SET password = $1, verify_token = NULL, verify_token_expire = NULL WHERE id = $2", [hashedPassword, userId]);
}
